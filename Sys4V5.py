...
results_all_df = pd.DataFrame(columns=['Run', 'Total Distance', 'Finished', 'Times Charged', 'Battery Threshold'])

# Define the range of battery thresholds
battery_thresholds = np.linspace(0.1, 1.0, 10)
...
for threshold in battery_thresholds:
    ...
    for run in range(300):  # Change the range to the number of simulations you want
        ...
        results_df = pd.concat([results_df, pd.DataFrame(
            {'Run': [run + 1], 'Total Distance': [total_distance], 'Finished': [empty],
             'Times Charged': [times_charged], 'Battery Threshold': [threshold]})])
...
# Plotting
for threshold in battery_thresholds:
    df = results_all_df[results_all_df['Battery Threshold'] == threshold]
    ...
    plt.title(f'Scatter Plot of Total Distance vs Times Charged (Battery Threshold: {threshold})')

    # Adding legend
    plt.legend(
        handles=[plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='blue', markersize=10, label='Finished'),
